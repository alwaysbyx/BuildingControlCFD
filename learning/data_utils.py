...
from models.mmgpt import GNOT, GNOTE

class LogLoss(_WeightedLoss):
    def __init__(self, d=2, p=2, regularizer=False, normalizer=None):
    ...
    def _lp_losses(self, g, pred, sigma, target):
        ...
        metrics = losses.mean().clone().detach().cpu().numpy() # REL L2
        log_term = self.sum_pool(g, torch.log(F.softplus(sigma) + 1e-6))
        quad_term = self.sum_pool(g, torch.square(pred - target) / F.softplus(sigma))
        loss = 0.5 * (log_term + quad_term).mean() + 1
        return loss, metrics

    def forward(self, g,  pred, sigma, target, x):
        loss, metrics = self._lp_losses(g, pred, sigma, target)
        if self.normalizer is not None:
            ori_pred, ori_target = self.normalizer.transform(pred,component=self.component,inverse=True), self.normalizer.transform(target, inverse=True) # raw deviation
            initial_condition = x[0].reshape(-1, 12)[:, -1:] * 400. + 400. # raw value
            ori_pred = ori_pred + initial_condition
            ori_target = ori_target + initial_condition
            loss2, metrics = self._lp_losses(g, ori_pred, sigma, ori_target)
        if self.regularizer:
...
def get_model(args):
    ...
    elif args.model_name == "GNOTE":
        return GNOTE(trunk_size=trunk_size + theta_size,branch_sizes=branch_sizes, output_size=output_size,n_layers=args.n_layers, n_hidden=args.n_hidden, n_head=args.n_head,attn_type=args.attn_type, ffn_dropout=args.ffn_dropout, attn_dropout=args.attn_dropout, mlp_layers=args.mlp_layers, act=args.act,horiz_fourier_dim=args.hfourier_dim,space_dim=args.space_dim,n_experts=args.n_experts, n_inner=args.n_inner)
...
class MIODataset(DGLDataset):
    ...
    def process(self):
        ...
        for i in range(len(self)):
            ...
            #print(x.shape, y.shape, u_p.shape, input_f[0].shape)
