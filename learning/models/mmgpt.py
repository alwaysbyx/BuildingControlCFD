...
class GNOTE(nn.Module):
    def __init__(self,
        ...
        self.out_mlp = MLP(n_hidden, n_hidden, output_size, n_layers=mlp_layers)
        self.out_mlp_sigma = MLP(n_hidden, n_hidden, output_size, n_layers=mlp_layers)

        self.__name__ = 'MIOEGPT_meanvariance'
    ...
    def forward(self, g, u_p, inputs):
        ...
        for block in self.blocks:
            x = block(x, z, pos)
        x_mean = self.out_mlp(x)
        x_mean_out = torch.cat([x_mean[i, :num] for i, num in enumerate(g.batch_num_nodes())],dim=0)
        x_sigma = self.out_mlp_sigma(x)
        x_sigma_out = torch.cat([x_sigma[i, :num] for i, num in enumerate(g.batch_num_nodes())],dim=0)
        return x_mean_out, x_sigma_out